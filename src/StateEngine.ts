...
export interface AgentSchema {
  ...
  compiler?: {
    bands?: Record<string, { low: string; mid: string; high: string }>;
...
export class StateEngine {
  ...
  tick(state: AgentState, kicks: Record<string, number> = {}): AgentState {
    ...
    const newNoise: Record<string, number> = {};
    ...
    for (const variable of this.schema.variables) {
      ...
      newValues[variable] = StateEngine.clamp01(
        x + lambda * (x0 - x) + coupling + kick + noise
      );
    }
  ...
  computeEffectiveBaseline(variable: string, nowMs?: number): number {
    ...
    if (!this.schema.circadian || variable !== 'energy') return baseline;
