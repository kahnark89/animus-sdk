...
class Compiler {
    ...
    circadianSentence(nowMs) {
        const hour = new Date(nowMs ?? Date.now()).getHours();
        if (hour >= 5 && hour < 9)
            return 'The morning is just starting.';
        if (hour >= 9 && hour < 12)
            return 'Morning energy is at its peak.';
        if (hour >= 12 && hour < 14)
            return 'Midday — focus comes naturally.';
        if (hour >= 14 && hour < 17)
            return "Afternoon's second wind.";
        if (hour >= 17 && hour < 21)
